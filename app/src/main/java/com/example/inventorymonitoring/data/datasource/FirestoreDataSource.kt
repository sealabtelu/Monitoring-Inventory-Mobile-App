package com.example.inventorymonitoring.data.datasource
...
class FirestoreDataSource(
    ...
    suspend fun updateBarang(dataBarang: DataBarang) {
        firestore.collection("data_barang").document(dataBarang.id)
            .set(dataBarang) // Use set to overwrite the document with the new data
            .await() // Await the completion of the operation
    }

    // Method to add a new item
    suspend fun addBarang(dataBarang: DataBarang) {
        firestore.collection("data_barang").add(dataBarang) // Use add to create a new document
            .await() // Await the completion of the operation
    }
        ...
        val items = snapshot.documents.mapNotNull { document ->
            ...
            dataBarang?.let {
                if (it.status == "Masuk") {
                    ...
                    Log.d("Umur", "Item: ${it.nama_barang}, Days Since Update: $umur")
    ...
    suspend fun getItemsWithEmptyNamaBarang(): List<DataBarang> {
        return try {
            val querySnapshot = firestore.collection("data_barang")
                .whereEqualTo("nama_barang", "")
                .get()
                .await()

            querySnapshot.documents.mapNotNull { document ->
                document.toObject(DataBarang::class.java)
            }
        } catch (e: Exception) {
            Log.e("FirestoreDataSource", "Error fetching items with empty nama_barang: ${e.message}")
        ...
        snapshot.documents.forEach { document ->
            ...
            dataBarang?.let {
                ...
                if (it.status == "Keluar") {
                    ...
                    val daysDifference = updatedDate?.let { (it.time - currentTime) / (1000 * 60 * 60 * 24) }?.toInt() ?: 0
